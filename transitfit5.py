...
def get_data_sol(koi_id,koicat,datadir,filedir,modeltype):
    ...
    phot,photflag=getphot(koi_dir,koi_id,koicat,idx,npl)
    #print('got phot..')
...
def getphot(koi_dir,koi_id,koicat,idx,npl,tdurcut=2):
    ...
    if os.path.isfile(nfile0) & os.path.isfile(photfile0):
        ...
        tdur=tfit5.transitdur(sol0,1)/86400.0/per #transit duration in days
    ...
    for line in f:
        ...
        if(columns[0][0] != '#'):
            if(icount > 0):  #skip header info
                ...
                koicat.rhostarm.append(float(columns[33]))
                koicat.rhostarmep.append(float(columns[34]))
                koicat.rhostarmem.append(float(columns[35]))
                ...
                koicat.rstar.append(float(columns[41]))
                koicat.rstar_ep.append(float(columns[42]))
                koicat.rstar_em.append(float(columns[43]))
                koicat.teff.append(float(columns[39]))
                koicat.teff_e.append(float(columns[40]))
                koicat.rhostar.append(float(columns[36]))
                koicat.rhostar_ep.append(float(columns[37]))
                koicat.rhostar_em.append(float(columns[38]))
                #if np.abs(float(columns[38]))> 1.0e-10:
                #    koicat.rhostar_em.append(float(columns[38]))
                #else:
                #    koicat.rhostar_em.append(float(-0.01))
                koicat.logg.append(float(columns[45]))
                koicat.logg_ep.append(float(columns[46]))
                koicat.logg_em.append(float(columns[47]))
                koicat.feh.append(float(columns[48]))
                koicat.feh_e.append(float(columns[49]))
                koicat.statusflag.append(columns[52])
            icount+=1
    f.close()
